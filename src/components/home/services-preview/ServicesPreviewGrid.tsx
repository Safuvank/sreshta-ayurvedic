...
export const ServicePreviewGrid = ({ limit }: ServicePreviewGridProps) => {
  ...
  return (
    <motion.div
      ...
      whileInView="show"
      viewport={{ once: true, margin: "-100px" }}
      // Changed to grid-cols-2 on mobile, smaller gap on mobile for better fit
    ...
    >
      {displayedServices.map((service, index) => (
        <motion.div 
          ...
          // Make the first item span full width on mobile (col-span-2), standard on desktop (md:col-span-1)
          className={`h-full ${index === 0 ? "col-span-2 md:col-span-1" : "col-span-1"}`}
        >
          <ServicePreviewCard {...service} index={index} />
        </motion.div>
      ))}
    </motion.div>
  );
