...
export const ServiceDetailHero = ({ service }: { service: ServiceProps }) => {
  return (
    <>
      {/* 1. Full-Width Hero Image Banner */}
      {/* FIXED: Adjusted padding for better mobile spacing while maintaining desktop height */}
      <section className="relative pt-28 md:pt-32 pb-16 md:pb-24 min-h-[50vh] lg:min-h-[60vh] flex items-end overflow-hidden">
        {/* Background Image & Overlay */}
        <div className="absolute inset-0 z-0">
          <Image
            src={service.image}
        ...
        <Container className="relative z-10 w-full">
          ...
          >
            <Link
              href="/services"
              className="inline-flex items-center gap-2 text-xs md:text-sm font-semibold tracking-widest text-[#C9A86A] uppercase hover:text-white transition-colors mb-6 md:mb-8"
            >
              <ArrowLeft className="w-4 h-4" />
              Back to Services
            </Link>
          ...
          <motion.h1 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.1 }}
            {/* FIXED: Scaled typography starting from text-4xl on mobile, added break-words and leading-tight */}
            className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-white mb-4 md:mb-6 leading-tight break-words"
          >
            {service.title}
          </motion.h1>
          ...
          <motion.p
            ...
            className="text-base sm:text-lg lg:text-xl text-gray-300 max-w-2xl font-light leading-relaxed"
          >
            {service.description}
          </motion.p>
        </Container>
