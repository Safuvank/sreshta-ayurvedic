...
export default function ChatWindow({
  ...
  return (
    <div 
      className="fixed z-[9999] flex flex-col overflow-hidden bg-white shadow-2xl transition-all duration-300
      /* Mobile layout (Bottom sheet stays attached to bottom) */
      bottom-0 right-0 w-full h-[85dvh] rounded-t-3xl 
      /* Desktop layout (Floats right above the new 35% button position) */
      sm:bottom-[calc(25%+4rem)] sm:right-[5%] sm:h-[500px] sm:w-[380px] sm:rounded-3xl"
    >
      <ChatHeader onClose={onClose} />
      <ChatMessages messages={messages} />
      <ChatInput onSend={onSend} />
